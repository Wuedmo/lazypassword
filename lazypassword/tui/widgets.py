...
class StatusBar(Widget):
    """Bottom bar showing current mode/key hints with responsive text truncation."""
    ...
    StatusBar #status-content Label {
        width: 1fr;
        min-width: 8;
        text-overflow: ellipsis;
        overflow: hidden;
    }
    /* Small screens: hide some labels */
    @media (max-width: 80) {
        StatusBar #status-encryption {
            display: none;
        }
    }
    @media (max-width: 60) {
    ...
    def compose(self):
        ...
        with Horizontal(id="status-content"):
            ...
            yield Static("", id="status-message", classes="message")
    ...
    def refresh_status(self):
        ...
        if self.message:
            message_label = self.query_one("#status-message", Static)
            message_label.update(self.message)
...
class EntryForm(Widget):
    """Form for creating/editing entries with responsive layout."""
...
class ConfirmDialog(Widget):
    """Yes/no confirmation dialog with responsive sizing."""
